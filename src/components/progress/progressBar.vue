<template>
  ...
  >
    <slot>
      ...
      <span v-if="computedShowProgress">{{
        computedProgress.toFixed(computedPrecision)
      }}</span>
      <span v-if="computedShowValue">{{
        value.toFixed(computedPrecision)
      }}</span>
    </slot>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    computedStriped() {
      return typeof this.striped === "boolean"
        ? this.striped
        : this.$parent.striped || false;
    },
    computedAnimated() {
      return typeof this.animated === "boolean"
        ? this.animated
        : this.$parent.animated || false;
    },
    computedMax() {
